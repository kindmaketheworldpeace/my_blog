...
class WMIEXEC:
    ...
    def run(self, addr):
        ...
        shell_output = self.shell.outputcontent
        self.shell = None
...
"""
    ...
    def do_cd(self, s):
        ...
        if len(self.__outputBuffer.strip('\r\n')) > 0:
            # diff: store output data
            self.outputcontent = self.__outputBuffer
            self.__outputBuffer = ''
        else:
...
def put_file(src_file, des_path, des_ip, username, password, domain="", share1="ADMIN$"):
    """上传文件"""
    try:
        cmd_str = "put " + str(src_file) + " " + str(des_path)
        executor = WMIEXEC(cmd_str, username, password, domain, share=share1)
        executor.run(des_ip)
        return {"result": True, "data": "upload %s success to %s:%s" % (src_file, des_ip, des_path)}
    except Exception as e:
        return {"result": False, "data": e.message if e.message else str(e)}


def execute_cmd(cmd_str, ipaddr, username, password, domain="", share1="ADMIN$"):
    """执行命令"""
    try:
        executor = WMIEXEC(cmd_str, username, password, domain, share=share1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-share', action='store', default='ADMIN$',
    ...
    try:
        ...
        if options.aesKey is not None:
            options.k = True
        print options.command, username, password, domain, options
