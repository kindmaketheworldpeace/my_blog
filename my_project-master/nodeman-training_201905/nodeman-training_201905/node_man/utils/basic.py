...
def parse_color(content):
    ...
    <span class="agent-color-green">ip</span>
    <span class="agent-color-gray">ip</span>
    <span class="agent-color-black">ip</span>
    """
...
def read_remote_file_content(remote_url):
    ...
    try:
        ...
        raise ValueError(u"无法通过内网URL {}找到初始化文件".format(remote_url))

    if response.status_code == 404:
        raise Http404(u"nginx服务器上找不到初始化文件")

    try:
        return response.json()
    except ValueError:
        raise ValueError(u"初始化文件格式错误，无法解析")
