...
class Account(AccountSingleton):
    ...
    BK_GET_USER_INFO_URL = "%s/login/api/v2/get_user/" % getattr(settings,
                                                                   'BK_PAAS_INNER_HOST',
                                                                   settings.BK_PAAS_HOST)
    ...
    def is_bk_token_valid(self, request):
        ...
        try:
            ...
            try:
                ...
                user.bk_supplier_account = data.get("bk_supplier_account", '0')
                user.bk_supplier_id = data.get("bk_supplier_id", 0)
                # 用户权限更新,保持与平台同步
    ...
    def _redirect_login(self, request, is_login=True):
        ...
        return redirect_to_login(callback, settings.LOGIN_URL,
